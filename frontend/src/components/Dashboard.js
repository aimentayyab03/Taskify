...
export default function Dashboard() {
  ...
  const [taskError, setTaskError] = useState(''); // <-- for empty field validation
  ...
  const addTask = async () => {
    if (!newTask.trim() || !category || !dueDate) {
      setTaskError('Please fill all fields before adding a task');
      return;
    }
          ...
          <>
            ...
            {view === 'all' && (
              <div className="add-task">
                {taskError && <p className="task-error">{taskError}</p>} {/* error message */}
